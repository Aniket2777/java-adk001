-- ============================================================
-- Shrija AI MCP Server Demo Data (MySQL)
-- ============================================================
...
SELECT 'EMP1024', 'Asha Rao', 'asha.rao@shrija.ai', 'Engineering', 'Senior Engineer', 'ACTIVE'
    WHERE NOT EXISTS (
    SELECT 1 FROM hr_employee WHERE employee_code = 'EMP1024'
);

INSERT INTO hr_employee (employee_code, full_name, email, department, designation, employment_status)
SELECT 'EMP2001', 'Vikram Shah', 'vikram.shah@shrija.ai', 'Finance', 'Analyst', 'ACTIVE'
    WHERE NOT EXISTS (
    SELECT 1 FROM hr_employee WHERE employee_code = 'EMP2001'
);

INSERT INTO hr_employee (employee_code, full_name, email, department, designation, employment_status)
SELECT 'EMP3050', 'Priya Menon', 'priya.menon@shrija.ai', 'Engineering', 'QA Engineer', 'ACTIVE'
    WHERE NOT EXISTS (
    SELECT 1 FROM hr_employee WHERE employee_code = 'EMP3050'
);
...
INSERT INTO employee_lifecycle_task
(employee_code, task_type, task_name, status, due_date)
SELECT
    'EMP3050',
    'ONBOARDING',
    'Laptop provisioning',
    ...
    WHERE NOT EXISTS (
    SELECT 1
    FROM employee_lifecycle_task
    WHERE employee_code = 'EMP3050'
      AND task_name = 'Laptop provisioning'
);
...
SELECT
    ...
    FROM employee_lifecycle_task
    WHERE employee_code = 'EMP3050'
      AND task_name = 'Access badge issued'
);
...
SELECT
    ...
    FROM employee_lifecycle_task
    WHERE employee_code = 'EMP3050'
      AND task_name = 'Benefits enrollment'
);
