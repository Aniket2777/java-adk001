...
public class ManagerAgentFactory implements AgentFactory {
    ...
    public ManagerAgentFactory(Gemini geminiModel, List<AgentFactory> departmentAgentFactories) {
        ...
        this.departmentAgentFactories = departmentAgentFactories.stream()
                .filter(factory -> !(factory instanceof ManagerAgentFactory))
                .toList();
    }

    @Override public String agentId() { return "manager-agent"; }

    @Override
    public BaseAgent build() {
        List<BaseAgent> subAgents = departmentAgentFactories.stream()
                .flatMap(factory -> {
                    try {
        ...
        return LlmAgent.builder()
                .name(agentId())
                .description("LLM orchestration agent that semantically routes requests to Employee, Attendance, Payroll or HR agents.")
