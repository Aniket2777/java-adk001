...
public class PayrollAgentFactory implements AgentFactory {
    ...
    public PayrollAgentFactory(Gemini geminiModel, @Qualifier("payrollMcpToolset") McpToolset payrollMcpToolset) {
        this.geminiModel = geminiModel;
        this.payrollMcpToolset = payrollMcpToolset;
    }
    @Override public String agentId() { return "payroll-agent"; }
    @Override public BaseAgent build() {
        return LlmAgent.builder()
                .name(agentId())
                .description("Handles employee salary, payslip, payroll history and latest salary requests using MCP.")
                .instruction("""
