...
public class AttendanceAgentFactory implements AgentFactory {
    ...
    public AttendanceAgentFactory(Gemini geminiModel, @Qualifier("attendanceMcpToolset") McpToolset attendanceMcpToolset) {
        this.geminiModel = geminiModel;
        this.attendanceMcpToolset = attendanceMcpToolset;
    }
    @Override public String agentId() { return "attendance-agent"; }
    @Override public BaseAgent build() {
        return LlmAgent.builder()
                .name(agentId())
                .description("Handles employee attendance, check-in, check-out and attendance history using MCP.")
                .instruction("""
