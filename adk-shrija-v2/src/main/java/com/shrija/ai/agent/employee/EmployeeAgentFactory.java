...
public class EmployeeAgentFactory implements AgentFactory {
    ...
    public EmployeeAgentFactory(Gemini geminiModel, @Qualifier("employeeMcpToolset") McpToolset employeeMcpToolset) {
        this.geminiModel = geminiModel;
        this.employeeMcpToolset = employeeMcpToolset;
    }
    @Override public String agentId() { return "employee-agent"; }
    @Override public BaseAgent build() {
        return LlmAgent.builder()
                .name(agentId())
                .description("Handles employee self-service: profile, leave balance/history/applications and document-related requests using MCP.")
                .instruction("""
