...
public class ConversationService {
    ...
    public ConversationResult converse(AuthenticatedUser user, @Nullable String sessionId, String message) {
        ...
        try {
            ...
            Flowable<Event> events = runner.runAsync(
                    userId, effectiveSessionId, userMessage, RunConfig.builder().build());
            return new ConversationResult(effectiveSessionId, collectFinalText(events));
        } catch (Exception ex) {
            log.error("Conversation turn failed for user={} session={}: {}", userId, effectiveSessionId, ex.getMessage(), ex);
