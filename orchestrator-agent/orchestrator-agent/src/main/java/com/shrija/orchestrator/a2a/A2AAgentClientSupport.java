 ...
 * prompt, and return its collected text response. Each downstream agent is resolved lazily on
 * every call (not at startup), so the Orchestrator can start even if not every domain agent
 * (Payroll, HR, Leave, Budget, CTO) is running yet -- a failed/unreachable agent surfaces as a
 * normal tool error the LLM can report, not a boot failure.
 */
@Component
public class A2AAgentClientSupport {
  ...
  public String call(String baseUrl, String prompt) {
    ...
    try {
      ...
      List<Event> events =
          runner.runAsync(userId, sessionId, message, RunConfig.builder().build()).toList().blockingGet();
