...
/**
 * The single entry point the React UI calls after storing its JWT, matching the program flow:
 * "ADK Service :8080 -> JWT Authentication Filter -> AuthenticatedUser -> Orchestration Agent".
 */
@RestController
...
public class OrchestratorController {
  ...
  public ResponseEntity<OrchestratorChatResponse> chat(
    ...
    return ResponseEntity.ok(new OrchestratorChatResponse(result.sessionId(), result.responseText()));
  }
}
